...
$serverList = Get-Content -Path C:\JenkinsScripts\PROD\Config\COMM_ListOfServers.txt

#get TH service name from config file
$CommService = Get-Content -Path C:\JenkinsScripts\PROD\Config\ATR_PROD_COMM_Service.txt
...
try
{
   $jobresult = Invoke-Command -Session $session -ScriptBlock {
            ...
            #get service TrueHome2CommunicationServiceHost in variable
         ...
         <#  fgdgd     if($service.Status -eq "Running") #to check if service is running
                {
                    #stopping service
                    $service.Stop()
